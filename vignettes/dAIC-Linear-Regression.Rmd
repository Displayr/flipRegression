...
\[
    \mathcal U(\boldsymbol{\theta}^*) = \mathbb{E}_g \left( \left.\frac{\partial \log f_{\boldsymbol{\theta}}(y|\boldsymbol{x})}{\partial \boldsymbol{\theta}}\right|_{\boldsymbol{\theta} = \boldsymbol{\theta}_0}\right) = 0.
\]

The likelihood equation, $\ell(\boldsymbol{\theta})$ being a population mean parameter can be estimated using a sample mean. In the context of sampling design, use the estimator,
\[
    \widehat{\mathcal U}(\boldsymbol{\theta}) = \frac{1}{N} \sum_{i = 1}^n w_i \ell_i(\boldsymbol{\theta})
\]
...
Then $\boldsymbol{\theta}^*$ is estimated by $\widehat{\boldsymbol{\theta}}$ that satisfies the is the weighted sample score equations, $\widehat{\mathcal U}(\widehat{\boldsymbol{\theta}}) = 0$
