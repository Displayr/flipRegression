...
* Properly define the semi-parametric approach, i.e. data follows $g(x, y)$ but modelling choice of $f_\theta(y|x)$ chosen and MLE used.
* Define $\theta^*$
* Reconcile the sampling design approach and results that seem invariant in the expectation.
* Add note about the rescaling of weights used in `survey` (i.e. $N = n$)
* Add detail of block-wise diagonal calculation, i.e. $\Delta$ is block-wise diagonal with $I_B V_B$ in the top left and $I_S V_S$ in the bottom right.
...
\[
    \boldsymbol{Y} \sim \mathcal N(\boldsymbol{X} \boldsymbol{\beta}_0, \sigma^2 \boldsymbol{I}) \quad \text{ or for an individual observation } \quad y \sim \mathcal N(\boldsymbol{x}^T \boldsymbol{\beta}_0, \sigma^2)
\]
...
Since the true parameters are unknown, the parametric family of models can be characterised in a parametric likelihood framework and the parameter vector estimated via maximisation. This is done as follows:
...
For convenience, define the unexplained variation as $\boldsymbol{\varepsilon} = \boldsymbol{Y} - \boldsymbol{X}\boldsymbol{\beta}_0 \sim \mathcal N(\boldsymbol{0}, \sigma_0^2 \boldsymbol{I})$.
...
\[
\begin{aligned}
    \frac{\partial \ell}{\partial \sigma^2} &= \frac{\partial l^{[1]}}{\partial \sigma^2} + \frac{\partial l^{[2]}}{\partial \sigma^2} + \frac{\partial l^{[3]}}{\partial \sigma^2}; \quad \frac{\partial{l^{[1]}}}{\partial \sigma^2} = 0\\
        &\Downarrow\\
...
In the complex survey design approach, the observations $\left\{ (y_i, \boldsymbol{x}_i): i \in s\right\}$ are assumed to be derived from a sample $s$ of $n$ units drawn from a finite population of size $N$ using some probability sampling design. The data from the finite population are assumed to be generated from some distribution with joint density, $g(y, \boldsymbol{x})$. The desired modelling approach for the data is assumed to be a parametric model, $\left\{ f_{\boldsymbol{\theta}}(y|\boldsymbol{x}), \boldsymbol{\theta} \in \boldsymbol{\Theta}\right\}$. Later the case of multiple regression is explored.
