...
test_that("Durbin Watson",
{
    ...
    y = x + rep(1:2,50)
    dat = data.frame(x, y)
    z = suppressWarnings(DurbinWatson((Regression(y ~ x, data = dat, type = "Binary Logit"))))
    expect_equal(z$statistic, c(d = 2.62), tolerance = 1.0e-3)
    expect_equal(z$p, 0, tolerance = 1.0e-5)
    z = DurbinWatson(suppressWarnings(Regression(y ~ x, data = dat)))
    expect_equal(z$statistic, c(d = 3.94), tolerance = 1.0e-3)
    expect_equal(z$p, 0, tolerance = 1.0e-3)
    # Positive autocorrelation
    y = x + c(rep(0,50),rep(1,50))
    dat = data.frame(x, y)
    z = DurbinWatson(suppressWarnings((Regression(y ~ x, data = dat, type = "Binary Logit"))))
    ...
    z1 <- DurbinWatson(suppressWarnings(Regression(y ~ x, data = dat)), n.permutations = 100000)
