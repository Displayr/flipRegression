...
#    body(.pairwise.regression)[n] <- NULL
    ...
    cors <- if (weighted)
        flipU::CovarianceAndCorrelationMatrix(y.and.x, weights, TRUE, TRUE)
    else
        cor(y.and.x, use = "pairwise.complete.obs")
    original <- #psych::
        setCor(1, 2:ncol(cors), data = cors, n.obs = min.pairwise.n, plot = FALSE, z = NULL, ...)
    result$original <- original
    scaled.beta <- as.matrix(original$beta)
    sds <- flipU::StandardDeviation(y.and.x, weights)
    ...
    mean.y <- flipU::Mean(y[resid.valid], weights[resid.valid])
    mean.y.hat <- flipU::Mean(fitted[resid.valid], weights[resid.valid])
    ...
    description <- flipU::BaseDescription(description,
...
#' @export
df.residual.RegressionCorrelations <- function(model, ...)
{
    model$original$df[2]
}
